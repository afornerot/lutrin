...
import { captureImageFromMedia, uploadCapturedImage, runOCR, runTTS, fetchTestTextFile } from '../services/processing.js';
...
async function startCaptureAndOCR() {
    ...
    try {
        ...
        const { blob, imageDataUrl } = await captureImageFromMedia(videoStream);
...
async function startTTSFromTextarea() {
    setCaptureButtonsState(true);
    audioPlayback.removeAttribute('src');
    clearStats();
    stopApiCheck();
    ...
    try {
        const textContent = ocrTextResult.value;
        if (!textContent || textContent.trim() === "") {
            showConsoleStatus("La zone de texte est vide. Saisissez du texte à lire.", true);
            setTimeout(hideConsoleStatus, 3000);
            return;
        }

        showConsoleStatus("Génération de l'audio (TTS) depuis le texte saisi...", false);
        const ttsStartTime = performance.now();
        const ttsData = await runTTS(textContent);
        const ttsEndTime = performance.now();
        ttsDuration = ttsEndTime - ttsStartTime;
        ...
        updateStats(null, null, ttsDuration);
        setCaptureButtonsState(false);
        startApiCheck();
    }
}
...
export function initConsoleView() {
    ...
    captureTextButtons = document.querySelectorAll('[id^="capture-text-"]'); // Note: le sélecteur est plus précis
    ...
    // Attacher l'événement pour le nouveau bouton
    document.getElementById('capture-textarea-button')?.addEventListener('click', startTTSFromTextarea);
}